...
{
    // Added CacheLength - viewport inflate multiplier
    // Composition scroll work asynchronously UI thread. 
    // Therefore, without additional elements, we can see empty areas when scrolling.
    public class VirtualizingStackPanel : VirtualizingPanel, IScrollSnapPointsInfo
    {
        ...
        public override IEnumerable<Control>? GetRealizedContainers()
        {
            return _realizedElements?.Elements.Where(x => x is not null)!;
        }

        public override Control? ContainerFromIndex(int index)
        {
        ...
        public override int IndexFromContainer(Control container)
        {
        ...
        public override Control? ScrollIntoView(int index)
        {
            var items = Items;
        ...
        private void OnEffectiveViewportChanged(object? sender, EffectiveViewportChangedEventArgs e)
        {
            ...
            var inflateEffectiveViewport = new Thickness(effectiveViewport.Width * cacheLength, effectiveViewport.Height * cacheLength);
            effectiveViewport = effectiveViewport.Inflate(inflateEffectiveViewport);

            _viewport = effectiveViewport.Intersect(new(Bounds.Size));
            _isWaitingForViewportUpdate = false;
